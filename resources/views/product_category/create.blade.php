            ...
            <div class="x_panel" style="border: 0px;">
                <div class="x_content">
                    ...
                    <form id="create-product-category" data-parsley-validate class="form-horizontal form-label-left" action=
                        "{{ Route('product-category.store',$request->rest_id) }}" method="POST" enctype="multipart/form-data">
                        {{csrf_field()  }}
                        <input type="hidden" name="rest_id" id="rest_id" value="{{ $request->rest_id }}" />
                        <input type="hidden" name="img" id="img"  />
						<input type="hidden" name="thumb_img" id="thumb_img"  />
                        <div class="item form-group">
                            <label class="col-form-label col-md-3 col-sm-3 label-align" for="cat_name">Category Name<span class="required">*</span></label>
                            <div class="col-md-6 col-sm-6 ">
                                <input type="text" id="cat_name" name="cat_name" required="required" class="form-control ">
                            </div>
                        </div>
                        <div class="item form-group">
                            <label class="col-form-label col-md-3 col-sm-3 label-align" for="ar_cat_name">Category Arabic Name<span class="required">*</span></label>
                            <div class="col-md-6 col-sm-6 ">
        ...
        var reader = new FileReader();
         ...
         reader.onloadend = function() {
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: '{{url("/product-category/upload_cat_thumbnail")}}',
                data: {'_token': $('meta[name="_token"]').attr('content'), 'image': base64data},
                success: function(data){
                    $modal.modal('hide');
                   /*  alert("success upload image"); */
